...
def sentence1() -> Expr:
    ...
    C = Expr('C')
    a_or_bexp = (A | B)
    nota_iff_notbc = ~A % (~B | C)
    nota_or_notbc = disjoin([~A, ~B, C])

    sentOne = conjoin([a_or_bexp, nota_iff_notbc, nota_or_notbc])
    return sentOne
    #return conjoin([c_iff_bc, a_imp_notbandd, notbc_imp_a, notd_imp_c])
    ###"*** END YOUR CODE HERE ***"
...
def sentence3() -> Expr:
    ...
    #isAlive = Expr("Pacman is alive at time")
    #born = Expr("Pacman is born")
    #killed = Expr("Pacman is killed")
    ...
    sent_one = PacmanAlive_1 % disjoin([(PacmanAlive_0 & ~PacmanKilled_0),
                                        (~PacmanAlive_0 & PacmanBorn_0)])
    sent_two = ~(PacmanAlive_0 & PacmanBorn_0)
    sent_three = PacmanBorn_0

    return conjoin([sent_one, sent_two, sent_three])


    #aliveOne = PacmanAlive_1
    #aliveZero = PacmanAlive_0
    #util.raiseNotDefined()
    ###"*** END YOUR CODE HERE ***"
...
def findModelCheck() -> Dict[Any, bool]:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    #util.raiseNotDefined()
    return [(dummyClass.__repr__(dummyClass('a')), True)]
    "*** END YOUR CODE HERE ***"
...
def entails(premise: Expr, conclusion: Expr) -> bool:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    sentence_entails = conjoin([~premise, conclusion])
    #cnf_entails = to_cnf(sentence_entails)
    if findModel(sentence_entails) == False:
        return True
    else:
        return False
    #return not findModel(sentence_entails)[sentence_entails]
        #get(sentence_entails)
        #(sentence_entails)
    #util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
